...
//a method called automatically to initialize the servlet
      ...
      throws ServletException, IOException
   {
      ...
      amount = req.getParameter( "DepositAmountTextField");
	...
	if(account[0].equals("Savings"))
  	{
		...
		Success = savingaccountbalance.deposit();
 	...
 	else
	{
		...
		Success = checkingaccountbalance.deposit();
		transaction newtransaction = new transaction(Username, amount, type);
		                 newtransaction.c_saveTransaction();
	}

	if(Success)
		output.println("Amount has been deposited successfully.");
	else
